    ...
    def send(self, vcc):
        try:
            ...
            for index in list(self._put.keys()):
                ...
                if vcc.api.put(f'/sumops/{record["ses_id"]}/{record["sta_id"]}', data=record):
                    self._put.pop(index)
            # send all delete records
            for index in list(self._delete.keys()):
                record = self._put[index]
                if vcc.api.delete(f'/sumops/{record["ses_id"]}/{record["sta_id"]}/{index}'):
...
class SessionReportWnd(tk.Tk):
    ...
    def get_session_info_from_vcc(self, schedule):
        ...
        if not (rsp := self.vcc.api.get(f'/sessions/{self.ses_id}')):
        ...
        if rsp := self.vcc.api.get(f'/sumops/{self.session.code}/{self.sta_id}'):
        ...
        if self.vcc is None:
            ...
            try:
                if not self.vcc.is_available:
                    self.vcc.connect()
                if rsp := self.vcc.api.put(f'/sumops/{self.session.code}/{self.sta_id}', data=record):
    ...
    def delete_vcc_record(self, record):
        if (rec_id := record.get('id', None)) and rec_id > 0:
            try:
                if not self.vcc.is_available:
                    self.vcc.connect()
                self.vcc.api.delete(f'/sumops/{self.session.code}/{self.sta_id}/{rec_id}')
                return
            except VCCError:
                pass

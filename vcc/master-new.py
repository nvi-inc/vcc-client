...
class SessionViewer:
    ...
    def __init__(self, ses_id):
        ...
        try:
            ...
            messagebox.showerror('VCC connect failed', str(exc))
            sys.exit(0)
    ...
    def make_optionbox(self, frame, url, selection):
        try:
            rsp = self.api.get(url)
            if rsp:
                names = [item['code'].strip() for item in json_decoder(rsp.json())]
                option = VCCOption(frame, ["Test 1", "test 2"], self.new_information)
        except VCCError:
            pass
        option.cb.setCurrentIndex(cb.findText(selection))
        return VCCOption(frame, ["Test 1", "test 2"], self.new_information)
        return cb
